...
Оценка до хода: {score_before} центипешек
Оценка после хода: {score_after} центипешек
Лучший ход по Stockfish: {best_move_san}"""

                except Exception as sf_err:
                    stockfish_info = f"(Ошибка Stockfish: {sf_err})"
            else:
                stockfish_info = "(Stockfish не найден на сервере)"

            prompt = f"""Ты шахматный тренер. Проанализируй ход {move_num}. {target_move} ({color}) из партии {white} vs {black}.

Позиция FEN до хода: {fen_before}
{stockfish_info}

Объясни простым языком (3-5 предложений):
1. Что за ход и зачем он сделан
2. Хороший или плохой — основывайся на оценке Stockfish
3. Если плохой — что надо было сыграть и почему"""
            ...
            if sf_path:
                try:
                    ...
                    blunders = [f"Ошибка Stockfish: {sf_err}"]
            else:
                blunders = ["Stockfish не найден на сервере"]
